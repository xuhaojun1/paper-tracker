...
def _render_card(it: Dict[str, Any],
    ...
    if has_any:
        out.append('<div class="section"><h4>Structured Analysis / 结构化分析</h4>')
        for label, en_key, zh_key in dims:
            ...
            if en_val or zh_val:
                ...
                if en_val: out.append(f"<div style='white-space:pre-wrap;margin-left:10px'>EN: {_esc(en_val)}</div>")
                if zh_val: out.append(f"<div style='white-space:pre-wrap;margin-left:10px'>ZH: {_esc(zh_val)}</div>")
        out.append('</div>')
